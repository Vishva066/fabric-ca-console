...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    clearContainers
    # Remove any volumes that are still present after the Compose shutdown
    ${CONTAINER_CLI} volume rm compose_orderer.example.com compose_peer0.org1.example.com compose_peer0.org2.example.com 2>/dev/null || true
    #Cleanup images
